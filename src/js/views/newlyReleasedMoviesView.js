...
class NewlyReleasedMoviesView extends View {
  _parentElement = document.getElementById(
    'newly-Released-swiper-parentElement'
  );
  _swiperContainer = document.getElementById(
    'newly-Released-movies-swiper-container'
  );
  _nextBtn = document.getElementById('newly_released_next');
  _prevBtn = document.getElementById('newly_released_prev');
  ...
  _generateMarkup() {
    return this._data
      .map(movie => {
        ...
        return `
                    <div class="swiper-slide movie-card flex-shrink-0 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 px-2" data-id="${movie.id}" data-type="tmdb" data-aos="fade-up">
                  <div
...
export default new NewlyReleasedMoviesView();
